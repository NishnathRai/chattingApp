...
function StatusPage(){
    ...
    function moveToViewStatusPage(){
        navigate(`viewStatus/MyStatus`);
    }
   ...
   <div className="out-status-box" >
       ...
       <div className="my-statux-box" >
        ...
        <>
        { status ? 
            <><button onClick={moveToViewStatusPage} className="but" >View My Status</button>
       ...
       <div className="status-bottom-box" >
            {data.map((val)=>{
                return  <Link  key={val.userId}  to={`viewStatus/${val.userId}`} ><StatusCard searchVal={search} key={val.userId} _id={val.userId} /></Link>;
            })}
       </div>
   </div>
   </>)
