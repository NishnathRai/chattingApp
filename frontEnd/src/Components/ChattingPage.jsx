...
function ChattingPage(){
    ...
    async function getMessages(count,add) {
        try{
           ...
           else loder.current.style.display = "display";
        }
    ...
    ////
    return (<>
    <div onKeyDown={(e)=>{ if( e.key=='Enter' ) sendMessage(typingMessage ,setTypingMessage,UserId,getMessages)}} className="chatting-page-box" >
        ...
        <div   className="main-messages" >
            ...
            {  messages.map((val,i)=>{
                return <Message messageType={val?.messageType} key={val._id} message={val?.message} time={convertToIST(val?.createdAt)} seen={val?.read} from={ val?.FromUserId!=UserId } _id={UserId} />;
            })  } 
        ...
        <div className="message-box" >
            ...
            <input ref={imageBox}  type="file" accept=".png, .jpg, .jpeg, .gif, .bmp, .webp" style={butStyle} placeholder="Send Image"  />
            <button onClick={()=>{sendImage(imageBox.current,UserId,getMessages)}} style={butStyle} >SendImage</button>
        </div>
    </div>
    </>);
}
